...
[In my last article, I shared how I made my site more accessible to use.](/blog/building-the-site) Today I'll talk about how I took Astro's original blog template, and customized the components to include some more personality and enhance the reading experience.

First, we'll look at what the template had to offer.

# Astro's blog template

Here's what the home page looked like below:
...
The blog page was also nice. It included a place for a large hero image on the top of the page, and the layout was nice to view on all screen sizes.
...
I kept the blog page the same for the most part. However, there were still some parts I wanted to change. In addition to the adjustments I made in my previous post, here are some components I modified and created to make my site better.
...
The HTML is pretty straightforward; I'm creating a container for the heading and the image to reside.
...
Although that's true, I wanted to have more control of the burger's position and rotation. The `background-position` CSS property only positions an image based on an X an Y coordinate relative to an element's container. I could move an image around, but not rotate it.

Having the image as its own element allowed me to move the burger freely. The CSS that styles the burger image looks like this:
...
With the burger in proper alignment, I decided to address the text next. I wanted "TASTY BYTES" to stand out from the rest of the heading. It's fun, and should look fun too. [Rodrigo Fuenzalida's Titan One font worked great.](https://www.dafont.com/titan-one.font?text=TASTY+BYTES)
...
Let's move closer to the blog itself by talking about the links that take readers to the posts. These are what I call "cards."

![Image of the original cards](@assets/blog/building-the-site-part-2/old-cards.png)

Cards present a way to show the reader what an article is about in a quick glance. Like video thumbnails, cards are also designed to draw a user's attention.

There are a couple of things that are wrong with this:

- I may not _want_ to provide an image to make it easier to write my blog entries.
- The title and content of the blog should be the center of attention, not an image.

Let's first see what happens when I remove an image from a blog post. Earlier, we noticed that a blog post with no hero image was still designed well. How does the card that links to that post look, then?

![Image of card with image and without image](@assets/blog/building-the-site-part-2/old-broken-card.png)

As you can see, the card for "First Post" renders a broken image. This is not good at all. I need a component that doesn't require an image and focuses on an article's content first.
...
These tags may seem abstract, but it's easiest to see the results once the site is published, and I post my link in different places. Here's an example of what card was rendered in Discord with minimal changes.
